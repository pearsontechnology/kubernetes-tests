        ...
        with open(hostYaml, 'r') as ymlfile1:  # hosts to test
            ...
            for host in contents['hosts']:
                if ("master" in host['name']):
                    ...
                    curlwrite = "curl -ks -X PUT -d 'test_value' https://consul.bitesize.${env}.${dom}/v1/kv/test/KEY1?token=${token}"
                    ...
                    curlread = "curl -ks https://consul.bitesize.${env}.${dom}/v1/kv/test/KEY1?token=${token} | jq '.[0].Value' | sed -e s/\"//g | base64 -d"
