...
def manually_verify_tokens():
    ...
    global readToken
    global writeToken
    cmd="ssh -q centos@{0} \'sudo kubectl get --no-headers secret vault-tokens-test-read --namespace=tokens-test -o json\' | jq -r \'.data[]\' | base64 -d".format(master)
    stdout,stderr,errorCode=run_script(cmd)
    assert errorCode == 0
    readToken=stdout.rstrip()
    uuidPattern.match(readToken)
    cmd="ssh -q centos@{0} \'sudo kubectl get --no-headers secret vault-tokens-test-write --namespace=tokens-test -o json\' | jq -r \'.data[]\' | base64 -d".format(master)
    stdout,stderr,errorCode=run_script(cmd)
    assert errorCode == 0
    writeToken=stdout.rstrip()
    uuidPattern.match(writeToken)
    cmd="ssh -q centos@{0} \'sudo su - -c \"vault token-lookup {1}\"\' | grep ^ttl | awk \'{{print $NF}}\'".format(master,readToken)
    stdout,stderr,errorCode=run_script(cmd)
    assert errorCode == 0
    readTokenTtl=stdout.rstrip()
    cmd="ssh -q centos@{0} \'sudo su - -c \"vault token-lookup {1}\"\' | grep ^ttl | awk \'{{print $NF}}\'".format(master,writeToken)
    stdout,stderr,errorCode=run_script(cmd)
    assert errorCode == 0
...
def check_ns_tokens():
    ...
    time.sleep(2)
    cmd="ssh -q centos@{0} \'sudo su - -c \"/usr/local/bin/renew_vault_tokens.sh check tokens-test\"\' | grep ^TTL\: | awk \'{{print $2}}\' | head -1".format(master)
    stdout,stderr,errorCode=run_script(cmd)
    assert errorCode == 0
    readTokenTtlNow=stdout.rstrip()
    cmd="ssh -q centos@{0} \'sudo su - -c \"/usr/local/bin/renew_vault_tokens.sh check tokens-test\"\' | grep ^TTL\: | awk \'{{print $2}}\' | tail -1".format(master)
    stdout,stderr,errorCode=run_script(cmd)
    assert errorCode == 0
...
def regen_tokens():
    ...
    cmd="ssh -q centos@{0} \'sudo su - -c \"vault token-lookup {1}\"\' | grep ^ttl | awk \'{{print $NF}}\'".format(master,readTokenNew)
    stdout,stderr,errorCode=run_script(cmd)
    assert errorCode == 0
    readTokenNewTtl=stdout.rstrip()
    cmd="ssh -q centos@{0} \'sudo su - -c \"vault token-lookup {1}\"\' | grep ^ttl | awk \'{{print $NF}}\'".format(master,writeTokenNew)
    stdout,stderr,errorCode=run_script(cmd)
    assert errorCode == 0
